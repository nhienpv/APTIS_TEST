...
def extract_task1_from_text(text):
    ...
    selects = re.findall(select_pattern, text, re.DOTALL)
    
    # Tìm các câu có select
    lines = text.split('\n')
    for line in lines:
        if '<SELECT>' in line:
...
def extract_task4_from_text(text):
    ...
    for line in lines:
        ...
        if line in ['A', 'B', 'C', 'D']:
            current_person = line
        elif current_person and line and not line.startswith(('Question', 'Four people', 'Correct answer')):
...
def extract_task5_from_text(text):
    ...
    for line in lines:
        line = clean_text(line)
        if line and not line.startswith(('Question', 'Choose a heading')):
            task5["topic"] = line
            break
